...
const threads = [
  {
    ...
    messages: [
      ...
      },
      {
        ...
        tags: [
          {
            label: "Human review recommended before changing benefit design.",
            type: "warning",
          },
        ],
      },
  ...
  },
  {
    ...
    messages: [
      ...
      },
      {
        ...
        tags: [
          {
            label: "Human review required before final rejection.",
            type: "warning",
          },
        ],
      },
          ...
          <div className="px-3 space-y-1 overflow-y-auto scrollbar-hide">
            {threads.map((thread, idx) => (
              <button
                ...
                className={`w-full text-left px-3 py-3 rounded-xl transition-all duration-300 flex flex-col gap-1 ${idx === activeThreadIndex
                  ? "bg-[#0048FF]/10 border border-[#0048FF]/30 shadow-[inset_0_0_20px_rgba(0,72,255,0.05)]"
                  : "hover:bg-white/5 border border-transparent"
                  }`}
              >
                <span
                  className={`text-xs font-semibold ${idx === activeThreadIndex ? "text-[#0048FF]" : "text-white/70"}`}
                >
                  {thread.title}
                </span>
          ...
          <header className="h-16 px-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-white/[0.01] backdrop-blur-md z-20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-[#0048FF] font-medium tracking-wide uppercase">
                  {activeThread.role}
                </span>
              </div>
            </div>
          ...
          >
            <AnimatePresence initial={false}>
              {visibleMessages.map((msg, i) => (
                ...
                >
                  <div
                    className={`p-4 rounded-2xl text-[13px] leading-relaxed shadow-sm ${msg.type === "user"
                  ...
                  {msg.tags && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {msg.tags.map((tag, idx) => (
                        <div
                          key={idx}
                          className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[9px] font-bold uppercase tracking-wider border ${tag.type === "confidence"
                            ? "bg-[#0048FF]/10 text-[#0048FF] border-[#0048FF]/30"
                            : tag.type === "warning"
            ...
            {activeRoleTyping === "ai" && (
              ...
              >
                <div className="flex items-center gap-2 p-4 rounded-2xl bg-[#11111A] rounded-tl-sm border border-white/5">
                  ...
                  <span className="text-xs text-[#0048FF] font-medium tracking-wide">
                    Processing logic...
                  </span>
                </div>
              </motion.div>
              ...
              <div className="flex-1 flex items-center pr-4">
                {activeRoleTyping === "user" ? (
                  ...
                  <span className="text-white/20 text-xs tracking-wide">
                    {activeRoleTyping === "ai"
                      ? "DeXa is responding..."
                      : "DeXa is managing this conversation..."}
                  </span>
                )}
              </div>
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors shrink-0 ${activeRoleTyping === "user"
                  ? "bg-[#0048FF]"
                  : "bg-[#0048FF]/20"
                  }`}
              >
                <Send
                  className={`w-4 h-4 ${activeRoleTyping === "user" ? "text-white" : "text-[#0048FF]"}`}
                />
              </div>
            </div>
          </div>
