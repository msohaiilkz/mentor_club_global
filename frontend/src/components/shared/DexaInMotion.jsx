...
const scenarios = [
  {
    ...
    outputs: [
      ...
      {
        label: "Executive Dashboard Updated",
        status: "Synced",
        icon: Database,
      },
    ],
...
const MatrixColumn = ({ active, xOffset }) => {
  ...
  useEffect(() => {
    setChars(
      Array.from({ length: config.length }).map(() =>
...
const MatrixRain = ({ active }) => {
  return (
    <div
      className={`absolute inset-0 overflow-hidden pointer-events-none transition-opacity duration-1000 ${active ? "opacity-50" : "opacity-0"}`}
    >
      {[...Array(50)].map((_, i) => (
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 md:px-12">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-base text-[#8A8A93] max-w-2xl mx-auto">
            DeXa is not just a chat layer. It is the intelligence engine that
            understands context, applies rules, connects systems, routes
            workflows, and guides action.
          </p>
        </div>

        {/* 3-Column Theatre Canvas */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-[420px] sm:min-h-[520px] lg:min-h-[600px] relative">
          {/* Left Panel: Conversation */}
          <div className="lg:col-span-4 flex flex-col h-full relative z-10 min-w-0">
            {/* Active Glow */}
            <div
              className={`absolute -inset-10 bg-[#0048FF]/20 blur-3xl rounded-full transition-opacity duration-1000 pointer-events-none -z-10 ${phase > 0 && phase < 5 ? "opacity-60" : "opacity-0"}`}
            />
                ...
                {phase >= 1 && (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5 px-1">
                      <span className="text-[9px] font-bold text-[#8A8A93] uppercase tracking-wider">
                        {scenario.role}
                      </span>
                ...
                {phase >= 2 && (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5 px-1">
                      ...
                      <span className="text-[9px] font-bold text-[#0048FF] uppercase tracking-wider">
                        DeXa Core
                      </span>
                    </div>
                    ...
                    <div className="bg-[#0048FF]/10 border border-[#0048FF]/20 rounded-lg rounded-tl-none px-4 py-3 max-w-[90%]">
                      <p className="text-xs text-white/80 leading-relaxed">
                        {phase === 4 ? (
                          <TypewriterText
                            text={scenario.a2}
                            onComplete={() => advancePhase(4, 1000)}
                            speed={15}
                          />
                        ) : (
                          scenario.a2
          ...
          <div className="lg:col-span-4 flex flex-col items-center justify-center relative py-10 sm:py-12 rounded-[2.5rem] border border-white/[0.05] bg-gradient-to-b from-[#0a0a14] to-[#05050A] shadow-[inset_0_0_80px_rgba(0,72,255,0.03)] overflow-hidden z-10 min-w-0">
            ...
            {/* Central Glow Pulse */}
            <div
              className={`absolute inset-0 bg-[#0048FF]/5 blur-3xl rounded-full transition-opacity duration-700 pointer-events-none ${phase === 5 ? "opacity-100" : "opacity-0"}`}
            />
            ...
            <div className="relative w-full max-w-[280px] flex flex-col items-center mt-6">
              ...
              {engineNodes.map((node, i) => {
                ...
                const isPassed =
                  phase > 5 || (phase === 5 && activeEngineNode > i);
                const isLineActive =
                  phase > 5 || (phase === 5 && activeEngineNode >= i + 1);

                return (
                  <div
                    key={node}
                    className="relative w-full flex justify-center mb-6 last:mb-0"
                  >
                    {/* The curved connecting line to the next node */}
                    {i < engineNodes.length - 1 && (
                      <div
                        className={`absolute top-1/2 w-8 ${i % 2 === 0
                          ...
                          } z-0 transition-all duration-300 ${isLineActive
                            ? "drop-shadow-[0_0_8px_rgba(0,72,255,0.8)]"
                            : ""
                          }`}
                        style={{
                    ...
                    >
                      ...
                      <span
                        className={`text-[11px] font-bold tracking-widest uppercase transition-colors relative z-10
                      ...
                      <div className="relative z-10 flex items-center">
                        {isPassed && (
                          <CheckCircle2 className="w-3.5 h-3.5 text-[#0048FF]" />
                        )}
                        {isActive && (
          ...
          <div className="lg:col-span-4 flex flex-col h-full relative z-10 min-w-0">
            {/* Active Glow */}
            <div
              className={`absolute -inset-10 bg-[#0048FF]/20 blur-3xl rounded-full transition-opacity duration-1000 pointer-events-none -z-10 ${phase === 6 ? "opacity-60" : "opacity-0"}`}
            />
            <MiniIpad>
              <div className="h-12 border-b border-white/5 flex items-center px-4 bg-white/[0.02] relative z-10">
                <div className="flex items-center gap-2 mt-2">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-[#8A8A93] uppercase tracking-widest">
                                {output.status}
                              </span>
                            </div>
                          </div>
