...
export function PhoneMockup({ children, className = "" }) {
  return (
    <div className={`relative w-[280px] sm:w-[320px] mx-auto ${className}`}>
      {/* Outer Phone Frame */}
      <div 
        className="relative z-10 bg-[#0A0A0A] rounded-[3.5rem] p-3 shadow-2xl border-[4px] border-[#1A1A1A] overflow-hidden aspect-[9/19]"
      ...
      >
        ...
        {/* Inner Screen */}
        <div 
          className="relative bg-black rounded-[2.8rem] overflow-hidden w-full h-full border border-white/5"
          style={{ WebkitMaskImage: "-webkit-radial-gradient(white, black)", WebkitTransform: "translateZ(0)" }}
        >
